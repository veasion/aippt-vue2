<template>
    <div class="outline_div">
        <div class="subject_div">{{ outlineTree.name }}</div>
        <div v-for="(chapter, chapterIdx) in outlineTree.children" :key="version + '_' + chapterIdx">
            <div class="chapter_div">
                ...
                <div class="operate_div">
                    <span title="在上方新增同级" @click="operate(outlineTree.children, chapterIdx, 1)">=+</span>
                    <span title="在下方新增下属" @click="operate(outlineTree.children, chapterIdx, 2)">+</span>
                    <span title="删除" @click="operate(outlineTree.children, chapterIdx, 3)">x</span>
                </div>
            </div>
...
export default {
  ...
  methods: {
    appendMd(children) {
      ...
      for (let i = 0; i < children.length; i++) {
          ...
          if (children[i].children) {
              str += this.appendMd(children[i].children)
          }
      }
      return str
    },
    operate(children, idx, type) {
    ...
    update() {
      let outlineMd = ''
      outlineMd += '# ' + this.outlineTree.name + '\n'
      outlineMd += this.appendMd(this.outlineTree.children)
      this.version++
      this.$emit('update', outlineMd)
    }
  }
}
</script>
